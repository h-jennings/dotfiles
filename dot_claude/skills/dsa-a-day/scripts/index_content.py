#!/usr/bin/env python3
"""
Index DSA problems and concepts by extracting YAML frontmatter.
...
def index_problems(repo_path):
    ...
    for problem_dir in sorted(problems_dir.iterdir()):
        ...
        if fm:
            ...
            fm["has_solution"] = (problem_dir / "solution.ts").exists()
            problems.append(fm)
...
def format_human_readable(items, content_type):
    ...
    # Summary stats
    if content_type == "problems" or content_type == "all":
...
def main():
    parser = argparse.ArgumentParser(description="Index DSA problems and concepts")
    parser.add_argument(
        "--type",
    ...
    if not repo_path.exists():
        print("No DSA repo found at ~/dsa-a-day/. Run setup.py first.", file=sys.stderr)
        return 1
