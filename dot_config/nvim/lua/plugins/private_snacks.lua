return {
	...
	keys = {
		...
		-- 	desc = "Open lazygit",
		-- },
		-- Deliberately parallel to Trouble's `<leader>xx`/`<leader>xX`, so the
		-- two can be compared side by side: a modal picker that closes on
		-- selection vs a persistent panel that drains as diagnostics are
		-- fixed. One of the two pairs should eventually be removed.
		{
			"<leader>xd",
			function()
